...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        r"""
        Args:
            sample (`torch.FloatTensor`): (batch, channel, f, height, width) noisy inputs tensor
        ...
        # if sample.shape[2] == 7 and timestep > 500:
        #     for i in range(5):
        #         lambda_1 = (5 - i) / 5
        #         lambda_2 = i / 5
        #         sample[:, :, i + 1] = lambda_1 * sample[:, :, 0] + lambda_2 * sample[:, :, -1]
            # sample[:, :, 1] = 0.5 * sample[:, :, 0] + 0.5 * sample[:, :, 2]
    ...
    def from_pretrained_2d(cls, pretrained_model_path, subfolder=None):
        ...
        #     if '.to_q' in k and 'attn1' in k:
        #         state_dict.update({k.replace('to_q', 'to_q2'): v})
